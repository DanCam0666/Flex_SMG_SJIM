...
namespace Flex_SGM.Controllers
{
    public class Parts_DeliveryController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var validMetricosRecords = metricos.Where(m => m.Usuario_area is "Parts_Delivery" && m.DiaHora.Year == DateTime.Now.Year).ToList();
            var users = validMetricosRecords.Where(m => m.Usuario_area is "Parts_Delivery")
            ...
            var chartLabel = metricos
                .Where(m => m.Usuario_area is "Parts_Delivery")
                .Where(m => m.DiaHora.Year == DateTime.Now.Year)
                .GroupBy(m => m.Usuario_responsable)
                .Select(m => m.Key)
                .ToList();
