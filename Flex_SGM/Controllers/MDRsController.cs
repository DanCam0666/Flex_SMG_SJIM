...
namespace Flex_SGM.Controllers
{
    public class MDRsController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var validMetricosRecords = metricos.Where(m => m.Usuario_area is "MDRs" && m.DiaHora.Year == DateTime.Now.Year).ToList();
            var users = validMetricosRecords.Where(m => m.Usuario_area is "MDRs")
            ...
            var chartLabel = metricos
                .Where(m => m.Usuario_area is "MDRs")
                .Where(m => m.DiaHora.Year == DateTime.Now.Year)
                .GroupBy(m => m.Usuario_responsable)
                .Select(m => m.Key)
                .ToList();
