...
namespace Flex_SGM.Controllers
{
    public class TOC_HS_AuditsController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var validMetricosRecords = metricos.Where(m => m.Usuario_area is "TOC_HS_Audits" && m.DiaHora.Year == DateTime.Now.Year).ToList();
            var users = validMetricosRecords.Where(m => m.Usuario_area is "TOC_HS_Audits")
            ...
            var chartLabel = metricos
                .Where(m => m.Usuario_area is "TOC_HS_Audits")
                .Where(m => m.DiaHora.Year == DateTime.Now.Year)
                .GroupBy(m => m.Usuario_responsable)
                .Select(m => m.Key)
                .ToList();
