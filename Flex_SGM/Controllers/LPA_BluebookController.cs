...
{
    public class LPA_BluebookController : Controller
    {
        ...
        public async Task<ActionResult> Index(string amaquina, string maquina, string submaquina, string mgroup, string xmgroup, string btn = "Metricos por Mes", string dti = "", string dtf = "")
        {
            ...
            var validMetricosRecords = metricos.Where(m => m.Usuario_area is "LPA_Bluebook" && m.DiaHora.Year == DateTime.Now.Year).ToList();
            var users = validMetricosRecords.Where(m => m.Usuario_area is "LPA_Bluebook")
            ...
            var chartLabel = metricos
                .Where(m => m.Usuario_area is "LPA_Bluebook")
                .Where(m => m.DiaHora.Year == DateTime.Now.Year)
                .GroupBy(m => m.Usuario_responsable)
                .Select(m => m.Key)
                .ToList();
