...
namespace Flex_SGM.Controllers
{
    public class ScrapController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var validMetricosRecords = metricos.Where(m => m.Usuario_area is "Scrap" && m.DiaHora.Year == DateTime.Now.Year).ToList();
            var users = validMetricosRecords.Where(m => m.Usuario_area is "Scrap")
            ...
            var chartLabel = metricos
                .Where(m => m.Usuario_area is "Scrap")
                .Where(m => m.DiaHora.Year == DateTime.Now.Year)
                .GroupBy(m => m.Usuario_responsable)
                .Select(m => m.Key)
                .ToList();
